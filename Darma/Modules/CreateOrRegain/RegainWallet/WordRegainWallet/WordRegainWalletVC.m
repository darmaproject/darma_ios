...
@interface WordRegainWalletVC ()<textViewEditDelegate,UITextFieldDelegate>
...
@property(nonatomic, strong)InputAutoChangeHeightView *wordKey;
...
@implementation WordRegainWalletVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    self.navigateView.title =BITLocalizedCapString( @"Recover from SEED words",nil);
...
- (void)addLayoutUI
{
    ...
    [self.view addSubview:self.wordKey];
    ...
    [_wordKey mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)regain:(UIButton *)sender{
    ...
    NSString *seed=[NSString stringWithFormat:@"%@",_wordKey.textView.text];
    ...
    if (seed.length<=0) {
        [SHOWProgressHUD showMessage:BITLocalizedCapString(@"Please enter SEED words",nil)];
        return;
    }
    NSString *fileNamePath = [filePath stringByAppendingString:wallteName];
    
    BOOL createWallet= [self.mobileWallet recovery_Encrypted_Wallet:fileNamePath password:password electrum_seed:seed];
    if (createWallet) {
        ...
        if (Height_Default) {
            NSLog(@"Set the default height for wallet scan-----------%i",(int)Height_Default);
        }
...
- (InputAutoChangeHeightView *)wordKey
{
    if (!_wordKey)
    {
        _wordKey = [[InputAutoChangeHeightView alloc] init];
        _wordKey.PromptLable.text=BITLocalizedCapString(@"25 Seed Words", nil);
        _wordKey.textView.placeholderLabel.text=BITLocalizedCapString(@"25 Seed Words", nil);
    }
    return _wordKey;
}
- (UIButton *)regainBtn
