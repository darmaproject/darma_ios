...
@interface RegainReadOnleyWalletVC ()<textViewEditDelegate,UITextFieldDelegate>
...
@property(nonatomic, strong)InputAutoChangeHeightView *publicKey;
...
@implementation RegainReadOnleyWalletVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    self.navigateView.title = BITLocalizedCapString(@"Recover View-only Wallet",nil);
...
- (void)addLayoutUI
{
    ...
    [self.view addSubview:self.publicKey];
...
-(void)regain:(UIButton *)sender{
    ...
    NSString *privateKey=[NSString stringWithFormat:@"%@%@",_publicKey.textView.text,_PrivateKey.textView.text];
    ...
    if (privateKey.length<=0) {
        [SHOWProgressHUD showMessage:BITLocalizedCapString(@"Private key can not be blank", nil) ];
        return;
    }
    NSString *fileNamePath = [filePath stringByAppendingString:wallteName];
    
    BOOL createWallet=   [self.mobileWallet create_Encrypted_Wallet_ViewOnly:fileNamePath password:password viewkey:privateKey];
    if (createWallet) {
        [[MobileWalletSDKManger shareInstance] closeWallet];
        [MobileWalletSDKManger shareInstance].mobileWallet=self.mobileWallet;
        NSLog(@"----------created new wallet file successfully");
        NSInteger Height_Default=  [self.mobileWallet  set_Initial_Height:[_startHeigt.textfiled.text intValue]];
        if (Height_Default) {
            NSLog(@"Set the default height for wallet scan-----------%li",Height_Default);
        }
        ...
        dic=@{@"name":name,@"time":time,@"address":address,@"status":@"1",@"fileNamePath":fileNamePath,@"type":@"Read only the wallet"};
...
- (InputAutoChangeHeightView *)publicKey
{
    if (!_publicKey)
    {
        _publicKey = [[InputAutoChangeHeightView alloc] init];
        _publicKey.PromptLable.text=BITLocalizedCapString(@"Spend public Key(64 hex characters)", nil);
        _publicKey.PromptLable.numberOfLines=0;
        _publicKey.textView.placeholderLabel.text=BITLocalizedCapString(@"Spend public Key(64 hex characters)", nil);
...
- (InputAutoChangeHeightView *)PrivateKey
{
    if (!_PrivateKey)
    {
        ...
        _PrivateKey.PromptLable.text=BITLocalizedCapString(@"View private Key(64 hex characters)", nil);
        _PrivateKey.PromptLable.numberOfLines=0;
        _PrivateKey.textView.placeholderLabel.text=BITLocalizedCapString(@"View private Key(64 hex characters)", nil);
    }
    return _PrivateKey;
}
- (UIButton *)regainBtn
